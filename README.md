...
| `--groupsel <list>` | Comma-separated Celestrak groups (e.g., `amateur,weather`, or `sunmoon`) | `active` |
...
### Available Satellite Groups (Celestrak)
Use these names with `--groupsel`:
*   **Special**: `active`, `visual`, `stations`, `last-30-days`, `analyst`, `sunmoon` (Sun & Moon only)
*   **Weather**: `weather`, `noaa`, `goes`, `resource`, `sarsat`, `dmc`, `tdrss`, `argos`, `planet`, `spire`
*   **Comm**: `geo`, `intelsat`, `ses`, `iridium`, `iridium-NEXT`, `starlink`, `oneweb`, `orbcomm`, `globalstar`, `swpc`, `amateur`, `x-comm`, `other-comm`, `satnogs`, `gorizont`, `raduga`, `molniya`
*   **Nav**: `gnss`, `gps-ops`, `glo-ops`, `galileo`, `beidou`, `sbas`, `nnss`, `musson`
*   **Science**: `science`, `geodetic`, `engineering`, `education`
*   **Misc**: `military`, `radar`, `cubesat`, `other`
